...
namespace SENSEI.BLL.AdminPortalService.Interface
{
    public interface IStudentPaymentService
    {
        ...
        Task<(IEnumerable<StudentBatchPayment>, long)> SearchStudentBatchPayment(long courseId = 0, long batchId = 0, string indexNumber = "", int status = -1, int start = 0, int length = 10, string searchValue = "", string sortColumn = "", string sortDirection = "");
        Task<StudentBatchPayment> GetStudentBatchPayment(long studentBatchPaymentId);
        Task<bool> ApproveStudentBatchPayment(long studentBatchPaymentId, long approvedById, DateTime paymentMonth, long? lessonId = null);

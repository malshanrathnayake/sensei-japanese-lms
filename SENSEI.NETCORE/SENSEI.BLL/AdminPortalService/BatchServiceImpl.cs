...
namespace SENSEI.BLL.AdminPortalService
{
    public class BatchServiceImpl : IBatchService
    {
        ...
        {
            DataTransactionManager dataTransactionManager = new DataTransactionManager(_databaseService.GetConnectionString());
            var batch = await dataTransactionManager.BatchDataManager.RetrieveData("GetBatch", new SqlParameter[] {
        ...
        {
            DataTransactionManager dataTransactionManager = new DataTransactionManager(_databaseService.GetConnectionString());
            var (batches, count) = await dataTransactionManager.BatchDataManager.RetrieveDataWithCount("SearchBatches", new SqlParameter[] {
        ...
        {
            DataTransactionManager dataTransactionManager = new DataTransactionManager(_databaseService.GetConnectionString());
            var batches = await dataTransactionManager.BatchDataManager.RetrieveData("GetBatch", new SqlParameter[] {
                new SqlParameter("@courseId", courseId),
            });
            return batches;
        }
        ...
        {
            DataTransactionManager dataTransactionManager = new DataTransactionManager(_databaseService.GetConnectionString());
            var status = await dataTransactionManager.BatchDataManager.DeleteData("DeleteBatch", new SqlParameter[] {
                new SqlParameter("@batchId", batchId),
            });
            return status;
        }
